...
router.get("/viewsql", (req, res) => {
  // passport.authenticate("jwt", { session: false }),
   ...
   pool.getConnection((err, conn) => {
     conn.query('SELECT linea as codigo,descripcion  FROM lineas order by linea DESC', (err, customers) => {
         if (err) {
             res.json(err);
         }
 ...
 router.post("/addlineasql", async (req, res) => {
  ...
  try {
      ...
      var caden="Insert into lineas(linea,descripcion,grupo,marca_eliminada) value ("+codigo+","+descripcion+",'1','NO'"+")";
...
router.delete(
 ...
 async  (req, res) => {
   ...
   var caden="DELETE FROM lineas WHERE linea ="+id;
 ...
 // passport.authenticate("jwt", { session: false }),
  async (req, res) => {
   ...
   // id=req.body.id;
    ...
    var caden="UPDATE  lineas SET descripcion="+descripcion+" "+ " WHERE linea ="+codigo;
