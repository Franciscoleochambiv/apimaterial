...
router.get("/viewsql", (req, res) => {
   ...
   pool.getConnection((err, conn) => {
     conn.query('SELECT AL_Id as codigo,AL_Descripcion as descripcion,direccion,provincia,ciudad,distrito,tel,empresa FROM almacenes order by AL_Id DESC', (err, customers) => {
         if (err) {
             res.json(err);
         }
 ...
 router.post("/addalmacensql", async (req, res) => {
  ...
  try {
      ...
      var caden="Insert into almacenes(AL_Id,AL_Descripcion,empresa,direccion,provincia,ciudad,distrito,tel) value   ("+codigo+","+descripcion+","+empresa+","+direccion+","+provincia+","+ciudad+","+distrito+","+tel +")";
...
router.delete(
 ...
 async  (req, res) => {
   ...
   var caden="DELETE FROM almacenes WHERE AL_Id ="+id;
